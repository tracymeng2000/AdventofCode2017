...
int diff(const vector< vector<int> > v, const int row){
	int largest = v[row][0];
	int smallest = v[row][0];
	cout << "initial largest = "<< largest << endl;
	cout << "initial smallest = " << smallest << endl;
	for(int i = 1; i < v[row].size(); i++){
		if(v[row][i] > largest){
			largest = v[row][i];
			cout << i << " " << "largest = " << largest << endl;
		}
		else if(v[row][i] < smallest){
			smallest = v[row][i];
			cout << i << " smallest = " << smallest << endl;
		}
	}
	cout << "\n\n\n";
...
int main(){
	vector< vector<int> > vtest = {{5,1,9,5}, {7,5,3}, {2,4,6,8}};
